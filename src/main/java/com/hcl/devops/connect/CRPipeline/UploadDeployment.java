...
public class UploadDeployment extends Builder implements SimpleBuildStep {
    ...
    throws AbortException, InterruptedException, IOException {
        if (!Jenkins.getInstance().getDescriptorByType(DevOpsGlobalConfiguration.class).isConfigured()) {
            listener.getLogger().println("Could not upload deployments to Accelerate as there is no configuration specified.");
            return;
        }
        ...
        listener.getLogger().println("Uploading deployment \"" + payload.get("version_name") + "\" of \"" + payload.get("name") + "\" to HCL Accelerate...");
        try {
            ...
            listener.getLogger().println("Successfully uploaded deployment to HCL Accelerate.");
        } catch (Exception ex) {
