...
public class UploadMetricsFile extends Builder implements SimpleBuildStep {
    ...
    throws AbortException, InterruptedException, IOException {
        if (!Jenkins.getInstance().getDescriptorByType(DevOpsGlobalConfiguration.class).isConfigured()) {
            listener.getLogger().println("Could not metrics file to Accelerate as there is no configuration specified.");
            return;
        }
        ...
        listener.getLogger().println("Uploading metric \"" + name + "\" to HCL Accelerate...");
        ...
        if (!success) {
            listener.getLogger().println("Problem uploading metrics file to HCL Accelerate.");
            ...
            listener.getLogger().println("Successfully uploaded metric file to HCL Accelerate.");
        }
    }
